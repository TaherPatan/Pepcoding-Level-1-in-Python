...
def mirror_trees(node1, node2):
    if len(node1.children) != len(node2.children):
        return False
    for (child1, child2) in zip(node1.children, reversed(node2.children)):
        if not mirror_trees(child1, child2):
            return False
    return True
...
if __name__ == '__main__':
    ...
    n = int(input())
    lst = list(map(int, input().split()))
    root2 = construct_generic_tree(lst)
    print(str(mirror_trees(root1, root2)).lower())
