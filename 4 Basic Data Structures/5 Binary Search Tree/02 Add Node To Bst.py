...
def add_node_bst(start_node, val):
    if not start_node:
        return Node(val)
    if val < start_node.data:
        start_node.left = add_node_bst(start_node.left, val)
    elif start_node.data < val:
        start_node.right = add_node_bst(start_node.right, val)
    return start_node
...
if __name__ == '__main__':
    ...
    root = add_node_bst(root, int(input()))
